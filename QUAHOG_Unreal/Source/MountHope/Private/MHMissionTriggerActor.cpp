    ...
    const FHitResult& SweepResult)
{
    ...
    AMountHopeGameMode* GameMode = GetWorld() ? Cast<AMountHopeGameMode>(GetWorld()->GetAuthGameMode()) : nullptr;
    if (!GameMode)
    {
        return;
    }

    const bool bInVehicle = OtherActor->IsA<AMountHopeVehiclePawn>();
