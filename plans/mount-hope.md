...
- **Cleanup (review §9/§10):** extracted the shared road-edge sampler `roadSamples.ts` (`sampleRoadEdges`) — the iterate-roads / skip-bridge / highway-whitelist / arc-length walk / sparse / radius-cull loop now lives in one place; `Decals` and `Graffiti` both call it and add their own jitter / perpendicular offset. `Graffiti` now buckets tags straight into per-variant arrays (dropped the redundant `byVariant` filter pass + the `v` field). Shipped (`c04cf3a`→HEAD).
